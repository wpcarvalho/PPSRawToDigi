 ...
 * Bits [19:21] 3 bits Roman Pot number 0= only RP
   Bits [17:18] 2 bits diamond plain 0,1,2,3
...
{  
 public:
  ...
  DiamondDetId(unsigned int Arm, unsigned int Station,
	     unsigned int RomanPot, unsigned int Plain, unsigned int Detector);
  ...
  inline int Plain() const
    {
      return int((id_>>startPlainBit) & 0x3);
    }
  ...
  inline unsigned int DetectorDecId() const
    {
      return Detector()+Plain()*100+RomanPot()*1000+Station()*10000+Arm()*100000;
    }
  ...
  static const unsigned int startPlainBit = 17;
 ...
 /// returns true if the raw ID is a PPS-timing one
  ...
  static unsigned int RawToDecId(unsigned int raw)
    {
      return ((raw >> startArmBit) & 0x1) * 100000 + ((raw >> startStationBit) & 0x3) * 10000 +
	((raw >> startRPBit) & 0x7) * 1000+((raw >> startPlainBit) & 0x3) * 100 + ((raw >> startDetBit) & 0x1F);
    }
  ...
  static unsigned int DecToRawId(unsigned int dec)
    {
      ...
      i &= 0xfe000000;
      i |= ((dec % 10) & 0xF) << startDetBit; dec /= 100;
      i |= ((dec % 10) & 0x3) << startPlainBit; dec /= 10;
  ...
  static unsigned int PlainOfDet(unsigned int i) { return i / 100; }
 ...
 private:
  inline void init(unsigned int Arm, unsigned int Station, unsigned int RomanPot, unsigned int Plain, unsigned int Detector);
};
