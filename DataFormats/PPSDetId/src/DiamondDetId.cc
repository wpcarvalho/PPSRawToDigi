...
{
  if( Arm>=2 || Station>=3 || RomanPot>=1 || Plain>3 || Detector>16)   //in timing station=1 we have one RP for timing therefore RomanPot>=1
    {
      throw cms::Exception("InvalidDetId") << "PPSTimingDetId ctor:" 
					   << " Invalid parameters: " 
					   << " Arm "<<Arm
  ...
  id_ |= ((Plain&0x3) << startPlainBit);
  id_ |= ((Detector&0xF) << startDetBit);
}


DiamondDetId::DiamondDetId(unsigned int Arm, unsigned int Station,
			       unsigned int RomanPot,unsigned int Plain, unsigned int Detector):       //third constructor with full id of all parts
  DetId(DetId::VeryForward, pps_timing_diamond_subdet_id)
{
  this->init(Arm,Station,RomanPot,Plain,Detector);
}
