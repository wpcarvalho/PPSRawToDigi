...
T1RoadProducerWOaplane::T1RoadProducerWOaplane(const ParameterSet& config):theVerbosity(0){
  ...
  thePlane2Skip = config.getParameter<int>("Plane2Skip");
  assert(thePlane2Skip>=0 && thePlane2Skip<5);
...
T1RoadProducerWOaplane::~T1RoadProducerWOaplane(){
...
void T1RoadProducerWOaplane::produce(Event& event, const EventSetup& setup) {
  //  cout << "Inside T1RoadProducerWOaplane::produce " << endl;
...
float T1RoadProducerWOaplane::Eta(float x,float y,float z){
  float xyt;
  float c=0;
  float eta2;
  xyt = sqrt(x*x + y*y);
...
float T1RoadProducerWOaplane::Phi(float x,float y){
  float c=0;
