...
T1TrackProducer3::T1TrackProducer3(const ParameterSet& config) :
	...
	theVerbosity(config.getParameter<int> ("Verbosity"))
{
	...
	if (theVerbosity >= 2) {
		std::cout << "Inside T1TrackProducer3" << std::endl;
	}
}

T1TrackProducer3::~T1TrackProducer3() {
}

void T1TrackProducer3::produce(Event& event, const EventSetup& setup) {
	...
	if (theVerbosity >= 2) {
		std::cout << "*************************************************************" << std::endl;
		std::cout << "\t\tT1TrackProducer3::produce" << std::endl;
		std::cout << "*************************************************************" << std::endl;
	}
...
int T1TrackProducer3::LookForTracks(T1Road road,	T1T2TrackCollection &track_collection) {
...
int T1TrackProducer3::Fit5PlaneTrack(list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road){

	double chi_square_over_n;
	double chi_square;
	list <Hits_pointer3> track_candidates;
			...
			for (list<int>:: iterator iter_2 = index_per_plane[2].begin(); iter_2 != index_per_plane[2].end(); iter_2++)
				for (list<int>:: iterator iter_3 = index_per_plane[3].begin(); iter_3 != index_per_plane[3].end(); iter_3++)
					...
					if ( TMath::Prob(chi_square,6.0) > theChiProbThr){
							track_candidates.push_back(	Hits_pointer3(hits_pointer_values, 5, chi_square_over_n) );
						}
					}
...
int T1TrackProducer3::Fit4PlaneTrack(list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road){
				...
				for (list<int>:: iterator iter_2 = index_per_plane[combinations[index][2]].begin(); iter_2 != index_per_plane[combinations[index][2]].end(); iter_2++)
					for (list<int>:: iterator iter_3 = index_per_plane[combinations[index][3]].begin(); iter_3 != index_per_plane[combinations[index][3]].end(); iter_3++){
						...
						if (TMath::Prob(chi_square,4.0) > theChiProbThr){
							track_candidates.push_back(	Hits_pointer3(hits_pointer_values, 4, chi_square_over_n) );
						}
					}
	return ObtainTracksFromCandidates(track_candidates, index_per_plane, track_collection, road, 4);
}

int T1TrackProducer3::Fit3PlaneTrack(list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road){
		...
		for (list<int>:: iterator iter_0 = index_per_plane[combinations[index][0]].begin(); iter_0 != index_per_plane[combinations[index][0]].end(); iter_0++)
			for (list<int>:: iterator iter_1 = index_per_plane[combinations[index][1]].begin(); iter_1 != index_per_plane[combinations[index][1]].end(); iter_1++)
				for (list<int>:: iterator iter_2 = index_per_plane[combinations[index][2]].begin(); iter_2 != index_per_plane[combinations[index][2]].end(); iter_2++){
					...
					//store the candidate
					if ( TMath::Prob(chi_square,2.0) > theChiProbThr){
						track_candidates.push_back(	Hits_pointer3(hits_pointer_values, 3, chi_square_over_n) );
					}
				}
	return ObtainTracksFromCandidates(track_candidates, index_per_plane, track_collection, road, 3);
}

int T1TrackProducer3::ObtainTracksFromCandidates(list<Hits_pointer3> track_candidates, list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road, int size){
	...
	if (theVerbosity >= 2 && (!track_candidates.empty())) {
		...
		for(list<Hits_pointer3>:: iterator iter=track_candidates.begin(); iter!=track_candidates.end(); iter++){
...
void T1TrackProducer3::ComputeParametersOfTrack(T1RecHitGlobal* hit_vector, int size, double* output_values){
...
T1T2Track T1TrackProducer3::fitTrack(T1RecHitGlobal* hitvec, int size) {

	if (size != 3 && size != 4 && size != 5 ){
		cout <<  "ERROR T1TrackProducer3::fitTrack : Number of hits: " << size << " should be 3,4 or 5 " <<endl  ;
		return T1T2Track();
	}
...
float T1TrackProducer3::Eta(float x, float y, float z) {
	float xyt;
	float c = 0.0;
...
float T1TrackProducer3::Phi(float x, float y) {
	float c = 0.0;
