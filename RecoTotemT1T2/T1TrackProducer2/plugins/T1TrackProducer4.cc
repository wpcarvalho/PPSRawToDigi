...
T1TrackProducer4::T1TrackProducer4(const ParameterSet& config) :
	theChiProbThr(config.getParameter<double> ("ChiProbThr")),
	theVerbosity(config.getParameter<int> ("Verbosity")),
	eVx(config.getParameter<double>("eVx")),
	eVy(config.getParameter<double>("eVy")),
	eVz(config.getParameter<double>("eVz"))
...

{
	...
	if (theVerbosity >= 2) {
		std::cout << "Inside T1TrackProducer4" << std::endl;
	}
}

T1TrackProducer4::~T1TrackProducer4() {
}

void T1TrackProducer4::produce(Event& event, const EventSetup& setup) {
	...
	if (theVerbosity >= 2) {
		std::cout << "*************************************************************" << std::endl;
		std::cout << "\t\tT1TrackProducer4::produce" << std::endl;
		std::cout << "*************************************************************" << std::endl;
	}
...
int T1TrackProducer4::LookForTracks(T1Road road,	T1T2TrackCollection &track_collection) {
...
int T1TrackProducer4::Fit5PlaneTrack(list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road){

	double chi_square_over_n;
	double chi_square;
	list <Hits_pointer4> track_candidates;
			...
			for (list<int>:: iterator iter_2 = index_per_plane[2].begin(); iter_2 != index_per_plane[2].end(); iter_2++)
				for (list<int>:: iterator iter_3 = index_per_plane[3].begin(); iter_3 != index_per_plane[3].end(); iter_3++)
					for (list<int>:: iterator iter_4 = index_per_plane[4].begin(); iter_4 != index_per_plane[4].end(); iter_4++){
						...
						chi_square = (values[0] + values[1]);
						chi_square_over_n = (values[0] + values[1]) / 8.0;

						if (theVerbosity >= 3) cout << "\t\tchi2 value of 5 hits track = " <<  chi_square_over_n << std::endl;

						//store the candidate
						if ( TMath::Prob(chi_square,8.0) > theChiProbThr){
							track_candidates.push_back(	Hits_pointer4(hits_pointer_values, 5, chi_square_over_n) );
						}
					}
...
int T1TrackProducer4::Fit4PlaneTrack(list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road){
			...
			for (list<int>:: iterator iter_1 = index_per_plane[combinations[index][1]].begin(); iter_1 != index_per_plane[combinations[index][1]].end(); iter_1++)
				for (list<int>:: iterator iter_2 = index_per_plane[combinations[index][2]].begin(); iter_2 != index_per_plane[combinations[index][2]].end(); iter_2++)
					for (list<int>:: iterator iter_3 = index_per_plane[combinations[index][3]].begin(); iter_3 != index_per_plane[combinations[index][3]].end(); iter_3++){
						...
						//store the candidate
						if (TMath::Prob(chi_square,6.0) > theChiProbThr){
							track_candidates.push_back(	Hits_pointer4(hits_pointer_values, 4, chi_square_over_n) );
						}
					}
	return ObtainTracksFromCandidates(track_candidates, index_per_plane, track_collection, road, 4);
}

int T1TrackProducer4::Fit3PlaneTrack(list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road){
		...
		for (list<int>:: iterator iter_0 = index_per_plane[combinations[index][0]].begin(); iter_0 != index_per_plane[combinations[index][0]].end(); iter_0++)
			for (list<int>:: iterator iter_1 = index_per_plane[combinations[index][1]].begin(); iter_1 != index_per_plane[combinations[index][1]].end(); iter_1++)
				for (list<int>:: iterator iter_2 = index_per_plane[combinations[index][2]].begin(); iter_2 != index_per_plane[combinations[index][2]].end(); iter_2++){
					...
					//store the candidate
					if ( TMath::Prob(chi_square,4.0) > theChiProbThr){
						track_candidates.push_back(	Hits_pointer4(hits_pointer_values, 3, chi_square_over_n) );
					}
				}
	return ObtainTracksFromCandidates(track_candidates, index_per_plane, track_collection, road, 3);
}

int T1TrackProducer4::ObtainTracksFromCandidates(list<Hits_pointer4> track_candidates, list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road, int size){
	...
	if (theVerbosity >= 2 && (!track_candidates.empty())) {
		...
		for(list<Hits_pointer4>:: iterator iter=track_candidates.begin(); iter!=track_candidates.end(); iter++){
...
void T1TrackProducer4::ComputeParametersOfTrack(T1RecHitGlobal* hit_vector, int size, double* output_values){

//	float x[5], y[5], z[5], ex[5], ey[5], ez[5];
	float x[6], y[6], z[6], ex[6], ey[6], ez[6];
	float Sx = 0;
	float Sy = 0;
	float Sxz = 0;
	float Szz_x = 0;
...
// impose vertex position and error
	x[size] = 0;
	y[size] = 0;
	z[size] = 0;
	ex[size] = eVx;
	...
	for (int index = 0; (index < size+1) && (index < 6); index++) {
	  if(index < size){
...
T1T2Track T1TrackProducer4::fitTrack(T1RecHitGlobal* hitvec, int size) {

	if (size != 3 && size != 4 && size != 5 ){
		cout <<  "ERROR T1TrackProducer4::fitTrack : Number of hits: " << size << " should be 3,4 or 5 " <<endl  ;
		return T1T2Track();
	}
...
float T1TrackProducer4::Eta(float x, float y, float z) {
	float xyt;
	float c = 0.0;
...
float T1TrackProducer4::Phi(float x, float y) {
	float c = 0.0;
