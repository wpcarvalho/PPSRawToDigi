...
#ifndef _T1TrackProducer2_h
#define _T1TrackProducer2_h
 ...
 *  Used by T1TrackProducer2 functions
 */
class Hits_pointer {
	public:
		 ...
		 */
		Hits_pointer(int* indexes, int size, double chi) {
			_chi = chi;
			_size = size;
		 ...
		 */
		bool operator <(const Hits_pointer& indici) {
		 ...
		 * @return Index of i-th element of the hit vector in road list. Road is a T1TrackProducer2::LookForTracks() member.
		 */
 ...
 */
class T1TrackProducer2: public edm::EDProducer {
	public:
		/**
		 *  Constructor.
		 */
		T1TrackProducer2(const edm::ParameterSet&);
		...
		virtual ~T1TrackProducer2();

		/**
		 *  The method which produces the reconstructed hits
		 *  @param event contains road collection. T1TrackProducer2::LookForTracks() is executed for every road in road collection.
	...
	private:
		 ...
		 *  @param track_candidates is a list of Hits_pointer. It contain all tracks with chiSquared / 2(sizeOfVector-2) lower than theChiRidThr set up in config file.
		 *  @param index_per_plane is a table of lists. index_per_plane[i] where (0 <= i <= 4) contains indexes of hits in road that are associated with the (i+1)-th plane of the detector.
		 *  @param track_collection is an output parameter. Best fitted tracks are stored in it.
		 *  @param road is a vector of T1RecHitGlobal. Many roads may be connected to one event.
		 ...
		 */
		int ObtainTracksFromCandidates(list<Hits_pointer> track_candidates, list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road, int size);
		/**
