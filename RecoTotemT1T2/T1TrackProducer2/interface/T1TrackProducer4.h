...
#ifndef _T1TrackProducer4_h
#define _T1TrackProducer4_h
 ...
 *  Used by T1TrackProducer4 functions
 */
class Hits_pointer4 {
	public:
		 ...
		 */
		Hits_pointer4(int* indexes, int size, double chi) {
			_chi = chi;
			_size = size;
		 ...
		 */
		bool operator <(const Hits_pointer4& indici) {
		 ...
		 * @return Index of i-th element of the hit vector in road list. Road is a T1TrackProducer4::LookForTracks() member.
		 */
 ...
 */
class T1TrackProducer4: public edm::EDProducer {
	public:
		/**
		 *  Constructor.
		 */
		T1TrackProducer4(const edm::ParameterSet&);
		...
		virtual ~T1TrackProducer4();

		/**
		 *  The method which produces the reconstructed hits
		 *  @param event contains road collection. T1TrackProducer4::LookForTracks() is executed for every road in road collection.
	...
	private:
		 ...
		 *  @param track_candidates is a list of Hits_pointer4. It contain all tracks with chiSquared / 2(sizeOfVector-2) lower than theChiRidThr set up in config file.
		 *  @param index_per_plane is a table of lists. index_per_plane[i] where (0 <= i <= 4) contains indexes of hits in road that are associated with the (i+1)-th plane of the detector.
		 *  @param track_collection is an output parameter. Best fitted tracks are stored in it.
		 *  @param road is a vector of T1RecHitGlobal. Many roads may be connected to one event.
		 ...
		 */
		int ObtainTracksFromCandidates(list<Hits_pointer4> track_candidates, list<int> index_per_plane[], T1T2TrackCollection& track_collection, T1Road road, int size);
		/**
