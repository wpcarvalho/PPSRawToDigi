...
T1MakeClusterMix::T1MakeClusterMix(const ParameterSet& config) :
  ...
  _ActivateDeadChannels(config.getParameter<bool>("ActivateDeadChannels"))
{
  ...
  if(theVerbosity >=1 )
{cout << " Inside T1MakeClusterMix "<<endl;
...
T1MakeClusterMix::~T1MakeClusterMix(){
...
void T1MakeClusterMix::produce(Event& event, const EventSetup& setup) {
...
//    event.getByType(Sdigis);
    event.getByLabel("T1AddMixedHits", "T1MixedHits", Sdigis);
...
float T1MakeClusterMix::Center(int fstrip, int lstrip, int * st){
...
float T1MakeClusterMix::Sigma(int fstrip, int lstrip, float center, int * st){
...
void T1MakeClusterMix::beginRun(edm::Run&, edm::EventSetup const& es){
  t1DataBeginJob(es);


}
void T1MakeClusterMix::t1DataBeginJob(const edm::EventSetup &es) 
{
