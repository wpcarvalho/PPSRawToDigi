...
void mergeNtuples(const int dOrbit, const string& totemFileName,
	...
	for (int i = 0; i < totemSize; ++i) {
		...
		if (it != totemOrbits.end()) { //if keys exist add to the existing vector
	...
	for (unsigned int cms_i = 0; cms_i < cmsSize; cms_i++) { //iterate over cms events, cms ntuples is not sorted
