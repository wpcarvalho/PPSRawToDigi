#ifndef T1_ANALOG_SIGNAL
#define T1_ANALOG_SIGNAL
...
class T1AnalogSignal
{
public:

  inline T1AnalogSignal() : theElement(0), invBinSize(0.), theBinValues(0), 
                               theTotal(0), theTimeOffset(0.) 
  {}
  
  inline T1AnalogSignal(int element, float binSize, std::vector<float> & binValues, float total=0., float timeOffset=0.)  :
...
//  } 
  ...
  inline void superimpose(const T1AnalogSignal & signal2) {
  ...
  friend std::ostream & operator<<(std::ostream &, const T1AnalogSignal &);
