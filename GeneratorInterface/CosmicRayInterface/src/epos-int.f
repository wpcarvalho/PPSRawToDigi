...
c----------------------------------------------------------------------
     ...
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
     ...
     .,pold,pnew,pnew2,p5a2,eeta,enf,pzf 
      ...
      double precision ptest(4),ttest,p52,p4mean(4,mxcl),zor,tor,xmxms
...
c...compute x,y,z
     ...
     .    ityptl(n).ge.40.and.ityptl(n).le.59.and.nnn.eq.1)then
           ...
           !print*,n,ityptl(n),idptl(iorptl(n)),istptl(iorptl(n)),nnn 
          endif
...
c     &               ,pptl(4,n)-delen
           ...
           if(iescape.eq.1)then  
              ...
              if(enf.gt.ena.or.enf.ne.enf)then
...
c...add segments to avoid holes
     ...
     .        .and.kk.ge.1.and.kk.le.m3grid)then
                 nplus=idropgrid(ii,jj,kk)+iflhc
                 if(nplus.gt.nsegce)then
                   ihit=1
                   goto 84
                 endif
              endif
   ...
   84   continue
        if(ihit.eq.1)then
          idropgrid(i,j,k)=idropgrid(i,j,k)+iflhc
...
c...absolute clusters numbering (for all k)
      ...
      if(iLHC.eq.-1)then    !fuse touching clusters along k
c        midbin=m3grid/2
...
c mimic boost transformation but protect against to high values of p(3) (p(3)~p(4)) 
               ...
               bet=bet+sqrt(pt2+pptl(5,ii)**2.
     *           *exp(-yradpp*pt2))
c     *          +(1.-abs(pptl(3,ii)/pptl(4,ii))**yradpp)
c     *                 *(pptl(5,ii)**2.+pptl(3,ii)**2))
...
c and pz :
     ...
     &         *(pt**2+pptl(3,j)**2)+pptl(5,j)**2))**fplmin)

                        yrad2(i)=min(1.,base+yrad2(i)) !should be here to avoid peal at eta=0.
                      else
                        yrad2(i)=1.
                        ytmp=yrad2(i)
                      endif
...
c          print *,'fin',niter,energ/tecm
                  if(abs(energ-tecm)/tecm.gt.0.05.and.niter.lt.1000)then
                    goto 611
                  elseif(niter.ge.1000)then
c      print *,'Rescaling failed:',pe(4),tecm,ptmax,plmax,ipart,ini0,ifi
                    if(ish.ge.1)write(ifch,*)'Random rescaling failed:'
...
c but only at 10% level to keep dependenc of sumEt with eta 
      if(tecm0.gt.0)then
        ini=nptlbcf+1
        ifi=nptl
        esoll=tecm0
