...
c---------------------------------------------------------------------
     ...
     &     ,multc1,multc83,multc25,rapgap,ipairs1,xsi
      parameter(mxxhis=70)
...
c...........check ids
          go=nidcod(n).eq.0
          do i=1,nidcod(n)
            if(istptl(j).eq.0.and.idcod(i,n).eq.9995)then    !all particles
...
c  or CMS hadron level (insdif=12)
...
c        x=max(0.,1.-x)
      elseif(inom.eq.351)then  !xsicms: CMS determination of xsi=M2_X/s
                               !using xsi=1 - Sum [(E+pz)_i/sqrt(s)]      where i runs on every reconstructed particles (= |eta|<4.9 charged + neutral)
        x=-2
c       pmax=sqrt((ecms/2.)**2-prom**2)
        pmax=pnullx               !???????????????????
        Ef=0.
