...
c string fragmentation
      ...
      wgtdiq=0.24      !weight for seadiq - antidiq as soft string end 
      fkainc=0.1   !factor for natural increase of string tension with energy
c energy dependence comes from fit of e+e->had mult in epos-fra
c this parameter is used to fix the increase of baryon and strangeness for Tevatron
      ...
      reminv=0.07   !remnant inversion probability (inversion important for forward pi(0) spectra : consequences on Xmax)

c radial flow (new)
      yradpx=0.5    !increase factor of low mass flow (conserv. mult.)
      yradpp=10.     !mass dependence of low mass flow (attenuation for heavy part)

c radial flow (old)
      yradmx=0.042   !increase factor of high mass flow (decrease. mult.)    
      yradpi=8.55      !minimum for rad flow (min=yradpi*yradmx)

c long flow
      ylongmx=-0.1  !long collective boost ( < 0 -> factor for mass dependence)
      yradmi= 8.55  !minimum for long flow (min=yradmi*ylongmx)
      ...
      iohole=1      !hole filling option (0=no)
      fploss=2.1     !parton energy loss (effective)
      fvisco=10.    !viscosity (effective) : transition from low mass (>0) to high mass (<0) flow
      fplmin=0.035     !fix the eta shape at high pt
      amuseg=2.75    !min mass for radial boost
      facecc=0.65   !eccentricity parameter
      nsegce=9      !number of segments per cell
...
c update file names
     ...
     & "Random number sequence does not start at 0 ... please wait !"
      ...
      if(model.ne.1.and.inicnt.eq.1)then
        ...
        if(model.eq.7)iversn=300 !'QGSJETII'
