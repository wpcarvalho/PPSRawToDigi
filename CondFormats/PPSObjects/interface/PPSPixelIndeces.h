#ifndef PPS_PIXELINDICES_H
#define PPS_PIXELINDICES_H
 ...
 *
 * In PPS  the row index is in the global y direction (local X) and
...
namespace {
  ...
  // Check the limits
  const bool PPS_CHECK_LIMITS = true;
}

class PPSPixelIndices {

 public:
  
  //*********************************************************************
  // Constructor with the ROC size fixed to the default.
   PPSPixelIndices(const int colsInDet,  const int rowsInDet ) : 
 ...
 void print(void) const {
  ...
  inline static int convertDcolToCol(const int dcol, const int pix, 
	...
	if(dcol<0||dcol>=DColsPerROC||pix<2||pix>161) {
	  std::cout<<"PPSPixelIndices: wrong dcol or pix "<<dcol<<" "<<pix<<std::endl;
	...
	if(colROC<0||colROC>=ROCSizeInY||rowROC<0||rowROC>=ROCSizeInX ) {
	  std::cout<<"PPSPixelIndices: wrong col or row "<<colROC<<" "<<rowROC<<" "
	      <<dcol<<" "<<pix<<std::endl;
	  rowROC = -1;    // dummy row Address
	  colROC = -1;    // dummy col Address
	  return -1;
 ...
 // At the moment this works only for modules read with a single TBM.
  int transformToModule(const int colROC,const int rowROC,
       ...
       if(PPS_CHECK_LIMITS) {
	if(colROC<0 || colROC>=ROCSizeInY || rowROC<0 ||rowROC>=ROCSizeInX) {
	  std::cout<<"PPSPixelIndices: wrong index "<<colROC<<" "<<rowROC<<std::endl;
	  return -1;
	}
      }
	...
	std::cout<<"PPSPixelIndices: wrong ROC ID "<<rocId<<std::endl;
	return -1;
      }
      if(PPS_CHECK_LIMITS) {
	if(col<0 || col>=(ROCSizeInY*theChipsInY) || row<0 || 
			     row>=(ROCSizeInX*theChipsInX)) {
	std::cout<<"PPSPixelIndices: wrong index "<<col<<" "<<row<<std::endl;
	return -1;
	}
      }
  ...
  int transformToROC(const int col,const int row,
	...
	if(col<0 || col>=(ROCSizeInY*theChipsInY) || row<0 || 
			     row>=(ROCSizeInX*theChipsInX)) {
	  std::cout<<"PPSPixelIndices: wrong index 3 "<<std::endl;
	  return -1;
	}
      }
      ...
      if(PPS_CHECK_LIMITS && (rocId<0 || rocId>=6) ) {
	std::cout<<"PPSPixelIndices: wrong roc index "<<rocId<<std::endl;
	return -1;
      }
  ...
  inline static int rocIndex(const int chipX, const int chipY) {
	...
	std::cout << "PPSPixelIndices: Error in ROC index " << rocId << std::endl;
	return -1;
      }
    }
    return rocId;
  ...
  inline static int DColumn(const int colROC) {
	...
	std::cout<<"PPSPixelIndices: wrong dcol index  "<<dColumnId<<" "<<colROC<<std::endl;
	return -1;
      }
    }
    return dColumnId;
