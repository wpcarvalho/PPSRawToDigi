...
void GenerateTrueMisalignmentsRP(unsigned int rp_id, FILE *f)
{
	...
	/*
	double si_rp_rot_xy = 0E-3;		// rad
	double si_rp_rot_z = 0E-3;		// rad

	double si_det_sh_xy = 0E-3;	// mm
	double si_det_rot_z = 0E-3;		// rad
	*/
	...
	for (unsigned int i = 0; i < 10; i++)
	{
		...
		fprintf(f, "\t\t<det id=\"%3u\" sh_x=\"%+.1f\" sh_y=\"%+.1f\" rot_z=\"%+.3f\" />\n", det_id, sh_x*1E3, sh_y*1E3, rot_z*1E3);
	}
}

//----------------------------------------------------------------------------------------------------

void GenerateErrorMisalignmentsRP(unsigned int rp_id, FILE *f)
{
	double si_rp_sh_x = 10E-3;		// mm
	double si_rp_sh_y = 10E-3;		// mm
	...
	/*
	double si_rp_sh_x = 0E-3;		// mm
	double si_rp_sh_y = 0E-3;		// mm
	double si_rp_rot_z = 0E-3;	// rad
	...
	*/
	...
	fprintf(f, "\n");
	fprintf(f, "\t<rp id=\"%3u\" sh_x=\"%+.1f\" sh_y=\"%+.1f\" rot_z=\"%+.3f\" />\n", rp_id, rp_sh_x*1E3, rp_sh_y*1E3, rp_rot_z*1E3);
...
int main(int argc, char **argv)
{
	...
	// true misalignments: for both real and misaligned geometries
	FILE *f_meas = fopen((outputLabel+"_real_misaligned.xml").c_str(), "w");
	fprintf(f_meas, "<xml DocumentType=\"AlignmentDescription\">\n");

	for (unsigned int i = 0; i < RPs.size(); i++)
		GenerateTrueMisalignmentsRP(RPs[i], f_meas);

	fprintf(f_meas, "</xml>\n");
	fclose(f_meas);

	// alignment error due to determination imperfections: for misaligned geometry only
	FILE *f_real = fopen((outputLabel+"_misaligned.xml").c_str(), "w");
	fprintf(f_real, "<xml DocumentType=\"AlignmentDescription\">\n");

	for (unsigned int i = 0; i < RPs.size(); i++)
		GenerateErrorMisalignmentsRP(RPs[i], f_real);
