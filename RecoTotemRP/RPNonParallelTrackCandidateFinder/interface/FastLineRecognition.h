...
class FastLineRecognition
{
  protected:
    ...
    bool GetOneLine(const std::vector<Point> &points, Cluster &result);

  public:
    /// \brief constructor, given cluster (full) widths and threshold
    FastLineRecognition(double cw_a = 0., double cw_b = 0., double _t = 0.);
    ...
    void GetLines(const std::vector<const RPRecoHit *> &input, double _z0, std::vector<RPRecognizedPatterns::Line> &lines);
};
