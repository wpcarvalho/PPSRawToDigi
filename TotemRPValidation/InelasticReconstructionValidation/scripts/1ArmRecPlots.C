...
void Draw1ArmPlots(void)
{
  ...
  SingleArmEntries(ksi_error, c1);
  c1->SaveAs("01_entries.gif");
  
  TCanvas *c2 = new TCanvas("Sigma(xi)", "Sigma(xi)");
  SingleArmSigma(ksi_error, "#sigma(#xi)", c2, true);
  c2->SaveAs("02_xi.gif");

  ReconstructionProfile *ksi_ref_error = (ReconstructionProfile *) gDirectory->Get("ksi_rel_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c2a = new TCanvas("Sigma(xi)/xi", "Sigma(xi)/xi");
  SingleArmSigma(ksi_ref_error, "#sigma(#xi)/#xi", c2a, true);
  c2a->SaveAs("03_xi_rel.gif");
  
  ReconstructionProfile *t_error = (ReconstructionProfile *) gDirectory->Get("t_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c3 = new TCanvas("Sigma(t)", "Sigma(t)");
  SingleArmSigma(t_error, "#sigma(t)", c3, true);
  c3->SaveAs("04_t.gif");
  
  ReconstructionProfile *t_rel_error = (ReconstructionProfile *) gDirectory->Get("t_rel_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c4 = new TCanvas("Sigma(t)/t", "Sigma(t)/t");
  SingleArmSigma(t_rel_error, "#sigma(t)/t", c4);
  c4->SaveAs("05_t_rel.gif");
  
  ReconstructionProfile *t_x_error = (ReconstructionProfile *) gDirectory->Get("t_x_error_vs_log_tx_log_ksi_phi_1");
  TCanvas *c4a = new TCanvas("Sigma(t_x)", "Sigma(t_x)");
  SingleArmSigma(t_x_error, "#sigma(t_{x})", c4a, true);
  c4a->SaveAs("06_tx.gif");
  
  ReconstructionProfile *t_x_rel_error = (ReconstructionProfile *) gDirectory->Get("t_x_rel_error_vs_log_tx_log_ksi_phi_1");
  TCanvas *c4b = new TCanvas("Sigma(t_x)/t_x", "Sigma(t_x)/t_x");
  SingleArmSigma(t_x_rel_error, "#sigma(t_{x})/t_{x}", c4b, true);
  c4b->SaveAs("07_tx_rel.gif");
  
  ReconstructionProfile *t_y_error = (ReconstructionProfile *) gDirectory->Get("t_y_error_vs_log_ty_log_ksi_phi_1");
  TCanvas *c4c = new TCanvas("Sigma(t_y)", "Sigma(t_y)");
  SingleArmSigma(t_y_error, "#sigma(t_{y})", c4c);
  c4c->SaveAs("08_ty.gif");
  
  ReconstructionProfile *t_y_rel_error = (ReconstructionProfile *) gDirectory->Get("t_y_rel_error_vs_log_ty_log_ksi_phi_1");
  TCanvas *c4d = new TCanvas("Sigma(t_y)/t_y", "Sigma(t_y)/t_y");
  SingleArmSigma(t_y_rel_error, "#sigma(t_{y})/t_{y}", c4d, true);
  c4d->SaveAs("09_ty_rel.gif");
  
  ReconstructionProfile *phi_error = (ReconstructionProfile *) gDirectory->Get("phi_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c5 = new TCanvas("Sigma(phi)", "Sigma(phi)");
  SingleArmSigma(phi_error, "#sigma(#phi) [rad]", c5, true);
  c5->SaveAs("10_phi.gif");
  
  ReconstructionProfile *vx_error = (ReconstructionProfile *) gDirectory->Get("vx_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c6 = new TCanvas("Sigma(vx)", "Sigma(vx)");
  SingleArmSigma(vx_error, "#sigma(vx)", c6);
  c6->SaveAs("11_vx.gif");
  
  ReconstructionProfile *vy_error = (ReconstructionProfile *) gDirectory->Get("vy_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c7 = new TCanvas("Sigma(vy)", "Sigma(vy)");
  SingleArmSigma(vy_error, "#sigma(vy)", c7);
  c7->SaveAs("12_vy.gif");
  
  ReconstructionProfile *chi = (ReconstructionProfile *) gDirectory->Get("chi2_error_overn_N_vs_log_t_log_ksi_phi_1");
  TCanvas *c8 = new TCanvas("chi2/ndf", "chi2/ndf");
  SingleArmSigma(chi, "#chi^{2}/ndf", c8);
  c8->SaveAs("13_chi2_ndf_means.gif");
  
  ReconstructionProfile *thetax = (ReconstructionProfile *) gDirectory->Get("thetax_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c8a = new TCanvas("thetax_error_vs_log_t_log_ksi_phi_1", "thetax_error_vs_log_t_log_ksi_phi_1");
  SingleArmSigma(thetax, "#sigma(#theta_{x})", c8a, true);
  c8a->SaveAs("14_thetax.gif");
  
  ReconstructionProfile *thetay = (ReconstructionProfile *) gDirectory->Get("thetay_error_vs_log_t_log_ksi_phi_1");
  TCanvas *c8b = new TCanvas("thetay_error_vs_log_t_log_ksi_phi_1", "thetay_error_vs_log_t_log_ksi_phi_1");
  SingleArmSigma(thetay, "#sigma(#theta_{y})", c8b);
  c8b->SaveAs("15_thetay.gif");
  ...
  chi2->Draw();
  c10->SaveAs("17_chi2ndf_dist.gif");
