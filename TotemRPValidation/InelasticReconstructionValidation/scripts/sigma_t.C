void sigma_t(ReconstructionProfile *p)
{
...
// sigma_xi_log_xi ************************************
   ...
   TCanvas *c1 = new TCanvas("c1","#sigma(t)(log #xi)",200,0,700,500);
   ...
   TH1F *hr1 = c1->DrawFrame(ximin-xistep,0,ximax+xistep,0.02);    
         hr1->SetXTitle("log #xi");hr1->SetYTitle("#sigma(t)");
   ...
   {   
       ...
       Char_t tvalue[64];                   // t value for legend
...
// sigma_xi_log_xi_log_t ************************************
   ...
   TCanvas *c2 = new TCanvas("c2","#sigma(t)(log #xi,log t)",200,0,700,500);
            c2->SetBorderMode(0);c2->SetFrameBorderMode(0);
            c2->SetLeftMargin(0.15);c2->SetRightMargin(0.05);

   TProfile2D *h = new TProfile2D("sigma(t)","",tbins,tmin-tstep,tmax+tstep,xibins,ximin-xistep,ximax+xistep,"s");   
               h->SetXTitle("log t");h->SetYTitle("log #xi");h->SetZTitle("#sigma(t)");
...
{
     Char_t tminvalue[64];sprintf(tminvalue, "%1.2f", tmin);
     Char_t tmaxvalue[64];sprintf(tmaxvalue, "%1.2f", tmax);
     Char_t ximinvalue[64];sprintf(ximinvalue, "%1.2f", ximin);
     Char_t ximaxvalue[64];sprintf(ximaxvalue, "%1.2f", ximax);
     Char_t phi1value[64];sprintf(phi1value, "%1.4f", phi1);
     Char_t phi2value[64];sprintf(phi2value, "%1.4f", phi2);
     Char_t tbinsvalue[64];sprintf(tbinsvalue, "%2.0f", tbins);
     Char_t tstepvalue[64];sprintf(tstepvalue, "%2.3f", tstep);
     Char_t xibinsvalue[64];sprintf(xibinsvalue, "%2.0f", xibins);
     Char_t xistepvalue[64];sprintf(xistepvalue, "%2.3f", xistep);
