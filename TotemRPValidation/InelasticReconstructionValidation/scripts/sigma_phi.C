void sigma_phi(ReconstructionProfile *p) {
...
// sigma_xi_log_xi ************************************
   ...
   TCanvas *c1 = new TCanvas("c1","#sigma(#phi)(log #xi)",200,0,700,500);
   ...
   TH1F *hr1 = c1->DrawFrame(ximin-xistep,0,ximax+xistep,0.02);    
         hr1->SetXTitle("log #xi");hr1->SetYTitle("#sigma(#phi)");
...
// sigma_xi_log_xi_log_t ************************************
   ...
   TCanvas *c2 = new TCanvas("c2","#sigma(#phi)(log #xi,log t)",200,0,700,500);
            c2->SetBorderMode(0);c2->SetFrameBorderMode(0);
            c2->SetLeftMargin(0.15);c2->SetRightMargin(0.05);

   TProfile2D *h = new TProfile2D("sigma(#phi)","",tbins,tmin-tstep,tmax+tstep,xibins,ximin-xistep,ximax+xistep,"s");   
               h->SetXTitle("log t");h->SetYTitle("log #xi");h->SetZTitle("#sigma(#phi)");
