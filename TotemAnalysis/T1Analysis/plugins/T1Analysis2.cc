...
class T1Analysis2 : public edm::EDAnalyzer {
public:
  explicit T1Analysis2(const edm::ParameterSet&);
  ~T1Analysis2();
...
//
T1Analysis2::T1Analysis2(const edm::ParameterSet& iConfig)
...
void
T1Analysis2::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
...
// multicounting delle tracce?
	if((*T1TC_it).Eta() > 3. && (*T1TC_it).Eta()<3.4){
	  hPhiPlus1->Fill( (*T1TC_it).Phi() );
	  hPhiDivPlus1->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiPlus1near->Fill( (*T1TC_it).Phi() );
hPhiDivPlus1near->Fill( (*T1TC_it).Phi() );
	}
	}

	if((*T1TC_it).Eta() > 3.4 && (*T1TC_it).Eta()<3.8){
	  hPhiPlus2->Fill( (*T1TC_it).Phi() );
	  hPhiDivPlus2->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiPlus2near->Fill( (*T1TC_it).Phi() );
hPhiDivPlus2near->Fill( (*T1TC_it).Phi() );
	}
	}
	if((*T1TC_it).Eta() > 3.8 && (*T1TC_it).Eta()<4.2){
	  hPhiPlus3->Fill( (*T1TC_it).Phi() );
	  hPhiDivPlus3->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiPlus3near->Fill( (*T1TC_it).Phi() );
hPhiDivPlus3near->Fill( (*T1TC_it).Phi() );
	}
	}
	if((*T1TC_it).Eta() > 4.2 && (*T1TC_it).Eta()<4.6){
	  hPhiPlus4->Fill( (*T1TC_it).Phi() );
	  hPhiDivPlus4->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiPlus4near->Fill( (*T1TC_it).Phi() );
hPhiDivPlus4near->Fill( (*T1TC_it).Phi() );
	}
	}

	if((*T1TC_it).Eta() < -3. && (*T1TC_it).Eta()>-3.4){
	  hPhiMinus1->Fill( (*T1TC_it).Phi() );
	  hPhiDivMinus1->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiMinus1near->Fill( (*T1TC_it).Phi() );
hPhiDivMinus1near->Fill( (*T1TC_it).Phi() );
	}
	}

	if((*T1TC_it).Eta() < -3.4 && (*T1TC_it).Eta()>-3.8){
	  hPhiMinus2->Fill( (*T1TC_it).Phi() );
	  hPhiDivMinus2->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiMinus2near->Fill( (*T1TC_it).Phi() );
hPhiDivMinus2near->Fill( (*T1TC_it).Phi() );
	}
	}
	if((*T1TC_it).Eta() < -3.8 && (*T1TC_it).Eta()>-4.2){
	  hPhiMinus3->Fill( (*T1TC_it).Phi() );
	  hPhiDivMinus3->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiMinus3near->Fill( (*T1TC_it).Phi() );
hPhiDivMinus3near->Fill( (*T1TC_it).Phi() );
	}
	}
	if((*T1TC_it).Eta() < -4.2 && (*T1TC_it).Eta()>-4.6){
	  hPhiMinus4->Fill( (*T1TC_it).Phi() );
	  hPhiDivMinus4->Fill( (*T1TC_it).Phi() );
	  if(Deta_max<0.6 && Dphi_max<0.1){hPhiMinus4near->Fill( (*T1TC_it).Phi() );
hPhiDivMinus4near->Fill( (*T1TC_it).Phi() );
	}
	}
...
T1Analysis2::beginJob()
{
  ...
  effplus = (new TH2D("effplus","effplus",120,0,2*PI,4,3,4.6));
  effminus = (new TH2D("effminus","effminus",120,0,2*PI,4,3,4.6));
  ...
  effplus ->SetDirectory(0); 
  effminus ->SetDirectory(0);
  ...
  for(unsigned int i=0;i<120;i++){
    ...
    for(unsigned int j=0;j<4;j++)
      {
	float eee=3.2 + (float)j*0.4;
	effplus->Fill(fi,eee,EFFplus[i][j]);
	effminus->Fill(fi,eee,EFFminus[i][j]);
...
T1Analysis2::endJob() 
{
  ...
  theFile = TFile::Open("T1Analysis2.root", "recreate");
...
void 
T1Analysis2::beginRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a run  ------------
void 
T1Analysis2::endRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when starting to processes a luminosity block  ------------
void 
T1Analysis2::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void 
T1Analysis2::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
T1Analysis2::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
