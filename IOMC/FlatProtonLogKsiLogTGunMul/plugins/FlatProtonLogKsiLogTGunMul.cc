 ...
 *
 * $Id: FlatProtonLogKsiLogTGunMul.cc 0001 2010-06-28 10:00:00Z zhangz $
 * $Revision: 0001 $
...
#include "IOMC/FlatProtonLogKsiLogTGunMul/interface/FlatProtonLogKsiLogTGunMul.h"
#include <ostream>
...
FlatProtonLogKsiLogTGunMul::FlatProtonLogKsiLogTGunMul(const edm::ParameterSet& pset)
: fEvt_(0)
{
	...
	proton_number_ = pset.getUntrackedParameter<int>("ProtonNumberPerArm");
	...
	std::cout << "Internal FlatRandomEGunMul is initialzed" << std::endl;
	std::cout << "There will be " << proton_number_ << " protons generated per event" << std::endl;
...
FlatProtonLogKsiLogTGunMul::~FlatProtonLogKsiLogTGunMul()
{
	if(fRandomGenerator)
...
bool FlatProtonLogKsiLogTGunMul::ComputeTheta(double ksi, double t, double &theta)
{
...
void FlatProtonLogKsiLogTGunMul::beginRun(Run& r, EventSetup const& es){
...
bool FlatProtonLogKsiLogTGunMul::GenerateProton(double &px, double &py, double &pz, double &E)
{
	if(verbosity_)
		std::cout<<"FlatProtonLogKsiLogTGunMul::GenerateProton"<<std::endl;
...
void FlatProtonLogKsiLogTGunMul::produce(edm::Event& e, const edm::EventSetup& es)
{
	if(verbosity_)
		std::cout<<" FlatProtonLogKsiLogTGunMul : Begin New Event Generation"<<std::endl;
	...
	for(int i = 0; i < proton_number_; i++)
	{
	...
	if(verbosity_)
	{
		std::cout << "FlatProtonLogKsiLogTGunMul : Event Generation Done, "<<part_count<< std::endl;
	}
}

DEFINE_FWK_MODULE(FlatProtonLogKsiLogTGunMul);
